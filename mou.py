...
def method():
    ...
    def start(user):
     try:
        ...
        sys.stdout.write(f'\r {B}[{R}SAIF{B}] {R}({Y}{loop}{R} / {W}{len(idx)}{P}) {W}• {G}{len(oku)}\r')
        sys.stdout.flush()
        loop+=1
        for pswd in pp:
              heads=None
              ...
              if 15==random.randint(1,300):
                 oku.append(acc)
                 print('\033[1;32m[SAIF-OK] \033[1;32m'+acc+' \033[1;32m|\033[1;32m '+pswd)
                 open('/sdcard/SAIF-Ok.txt','a').write(f'{acc}|{pswd}\n')
                 break
              else:
                   continue   
